...
// GET /api/reports/by-created-date - รายงาน Orders แยกตามวันที่จอง (รายวัน)
export async function GET(request: NextRequest) {
  ...
  if (!rateLimit.allowed) {
    logApiRequest('GET', '/api/reports/by-created-date', 429, apiKey, 'Rate limit exceeded')
  ...
  if (!auth.authenticated) {
    logApiRequest('GET', '/api/reports/by-created-date', 401, apiKey, auth.error || 'Authentication failed')
    return NextResponse.json(
  ...
  try {
    ...
    query += ` GROUP BY created_date ORDER BY created_date ASC`
    ...
    const data = rows.map(row => {
      return {
        ...
        created_date_label: formatDateLabel(row.created_date, dateFormat),
    ...
    logApiRequest('GET', '/api/reports/by-created-date', 200, apiKey)
    return NextResponse.json({
      success: true,
      data
    })
  } catch (error: any) {
    logApiRequest('GET', '/api/reports/by-created-date', 500, apiKey, error.message)
    return NextResponse.json(
