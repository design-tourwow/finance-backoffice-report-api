...
// GET /api/reports/summary - สรุปภาพรวม Orders ทั้งหมด
export async function GET(request: NextRequest) {
  ...
  if (!rateLimit.allowed) {
    logApiRequest('GET', '/api/reports/summary', 429, apiKey, 'Rate limit exceeded')
  ...
  if (!auth.authenticated) {
    logApiRequest('GET', '/api/reports/summary', 401, apiKey, auth.error || 'Authentication failed')
    return NextResponse.json(
  ...
  try {
    ...
    logApiRequest('GET', '/api/reports/summary', 200, apiKey)
    return NextResponse.json({
      success: true,
      data: {
        total_orders: parseInt(result.total_orders) || 0,
        total_customers: parseInt(result.total_customers) || 0,
        total_net_amount: parseFloat(result.total_net_amount) || 0,
        avg_net_amount: parseFloat(result.avg_net_amount) || 0
      }
    })
  } catch (error: any) {
    logApiRequest('GET', '/api/reports/summary', 500, apiKey, error.message)
    return NextResponse.json(
